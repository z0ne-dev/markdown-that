...
static HTTPS_OR_MAILTO: LazyLock<Regex> =
    LazyLock::new(|| Regex::new("(?i)^(https?:|mailto:)$").unwrap());

static IP_HOST_CHECK: LazyLock<Regex> = LazyLock::new(|| Regex::new(r#"\.\d"#).unwrap());
...
pub fn format_url_for_computers(url: &str) -> String {
    ...
    let encode = |s: String| crate::urlencode::encode(&s, ENCODE_DEFAULT_CHARS, true).to_string();
