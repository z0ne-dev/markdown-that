...
//!
//! 1. Literal marker character sequence can be used inside a structure if its length
//! doesn't match the length of the opening/closing sequence (e.g., with `%` defined
...
//!
use crate::parser::extset::{InlineRootExt, MarkdownThatExt};
...
impl<const MARKER: char> MarkdownThatExt for CodePairConfig<MARKER> {}

pub fn add_with<const MARKER: char>(md: &mut MarkdownThat, f: fn(length: usize) -> Node) {
...
impl<const MARKER: char> InlineRule for CodePairScanner<MARKER> {
    ...
    fn run(state: &mut InlineState) -> Option<(Node, usize)> {
        ...
        if backticks.scanned && backticks.max.get(opener_len).copied().unwrap_or(0) <= state.pos {
            // performance note: adding an entire sequence into pending is 5x faster,
            // but it will interfere with other rules working on the same char;
            // and it is extremely rare that a user would put a thousand "`" in text
            return None;
        }
        ...
        // Nothing found in the cache, scan until the end of the line (or until the marker is found)
